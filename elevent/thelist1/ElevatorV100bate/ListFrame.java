...
public class ListFrame extends JFrame implements Runnable, Publiclist {
	...
	public ListFrame() {
		...
		chooses[3].addActionListener(new ActionListener() // 制作退出按钮
		{
			public void actionPerformed(ActionEvent arg0) {
				System.exit(0);
			}
		...
		public void mousePressed(MouseEvent e) {
			for (int i = 0; i < upButton.length; i++) {
				if (e.getSource() == upButton[i]) {
					...
					if(upButton[i].getBackground()==pressedUpDownColor){}
					else{
				...
				if (e.getSource() == downButton[i]) {
					...
					if(downButton[i].getBackground()==pressedUpDownColor){}
					else {//System.out.println("☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆"+downState[i]);
	...
	public void aiControl() {
		...
		for (int i = aiDegree*20; i < (aiDegree*20+20); i++) {
			...
			listText.aiPeopleText[i%20][listText.listMeth][1] = 2;
			}
	...
	public void aiPeopleControl() {
		...
		for (int i = aiDegree*20; i < (aiDegree*20+20); i++) {
			...
			for (int j = 0; j < listNum; j++) {
				...
				listText.aiPeopleText[i][listText.listMeth][1] = 0;
				}
	...
	public listDialog(){
		...
		JTextArea listDialogJ = 
  new JTextArea("普通算法下   \t:" +
				"上行平均等待时间："+listText.listUpMax[0]+"\t下行平均等待时间："
				+listText.listDownMax[0]+"\t用户时间："+listText.aiPeopleAveMx[0]+"\n"+
				"特殊楼层算法下:\t " +
				"上行平均等待时间："+listText.listUpMax[1]+"\t下行平均等待时间："
				+listText.listDownMax[1]+"\t用户时间："+listText.aiPeopleAveMx[1]+"\n"+
				"奇偶算法下:\t " +
				"上行平均等待时间："+listText.listUpMax[2]+"\t下行平均等待时间："
				+listText.listDownMax[2]+"\t用户时间："+listText.aiPeopleAveMx[2]+"\n"+
				"高分楼层算法下:\t " +
				"上行平均等待时间："+listText.listUpMax[3]+"\t下行平均等待时间："
				+listText.listDownMax[3]+"\t用户时间："+listText.aiPeopleAveMx[3]+"\n"
				
		);
		container.add(listDialogJ);	
		setSize(600,200);
	}
