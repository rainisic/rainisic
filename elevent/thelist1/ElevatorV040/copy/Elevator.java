...
public class Elevator extends JFrame implements publicframe, ActionListener {
	...
	Elevator() {
		...
		aButtonState = new int[6];
		bButtonState = new int[6];
		frame.add(p);
	}
	...
	public void control() {
		if (getdirect() == 1) {
			...
			if (floormove % 40 == 0) {
				curfloor++;
				if (getdirect() == 0 ||upButtonState[curfloor-1] ==1 || aButtonState[curfloor-1]==1)
					openandclose = 1;
			...
			if (floormove % 40 == 0) {
				curfloor--;
				if (getdirect() == 0|| downButtonState[curfloor-1]==1||aButtonState[curfloor-1]==1)
					openandclose = 1;
				downButtonState[bcurfloor-1] =0;
			}
		} else if (getdirect() == 0 && doorabout == 1) {
		  	upButtonState[bcurfloor-1] =0;
			downButtonState[bcurfloor-1] =0;
	...
	public void bcontrol() {
		if (bgetdirect() == 1) {
			...
			if (bfloormove % 40 == 0) {
				bcurfloor++;
				if (bgetdirect() == 0 || upButtonState[bcurfloor-1]==1|| bButtonState[bcurfloor-1]==1)
					bopenandclose = 1;
				upButtonState[bcurfloor-1] =0;
			}
			...
			if (bfloormove % 40 == 0) {
				bcurfloor--;
				if (bgetdirect() == 0 || downButtonState[bcurfloor-1]==1|| bButtonState[bcurfloor-1]==1)
					bopenandclose = 1;
				downButtonState[bcurfloor-1] =0;
			}
		} else if (bgetdirect() == 0 && bdoorabout == 1) {
			upButtonState[bcurfloor-1] =0;
			downButtonState[bcurfloor-1] =0;
			...
			bf2.setBackground(Color.gray);	
			bf3.setBackground(Color.gray);	
			bf4.setBackground(Color.gray);	
			bf5.setBackground(Color.gray);	
			bf6.setBackground(Color.gray);
		}
	}
	...
	public void controller() {
		...
		for(int i=0;i<6;i++)
		{
			...
			if(aButtonState[i]==1){upmax=i+1; }
			if(bButtonState[i]==1){bupmax=i+1;}
		}
		if((getdirect() == 0 && openandclose == 0)&&bpurfloor!=upmax)
		{ purfloor = upmax; 
		}
		if((bgetdirect() == 0 && bopenandclose == 0)&&purfloor!=bupmax)
		{ bpurfloor = bupmax;
		}
		...
		if((bgetdirect() == 0 && bopenandclose == 0)&&purfloor!=bdownmax)
		{ bpurfloor = bdownmax;
		 }
